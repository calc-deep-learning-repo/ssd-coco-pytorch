...
if __name__ == "__main__":
    ...
    '''
    # Method 1: use torchvision API load dataset
    ...
    '''
    # Method 2: use user-defined API load dataset
    from coco2 import COCODetection
    ...
    # Method 3: use user-defined API load dataset
    from coco import COCODetection2, detection_collate
    from augmentations import SSDAugmentation
    from config import MEANS
    train_dataset = COCODetection2(root="./data/coco/", transform=SSDAugmentation(cfg['min_dim'], MEANS))
    val_dataset = COCODetection2(root="./data/coco/", transform=SSDAugmentation(cfg['min_dim'], MEANS))

    train_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=0, collate_fn=detection_collate, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size, shuffle=True, num_workers=0, collate_fn=detection_collate, drop_last=True)
    ...
    for epoch_idx in range(0, total_epoch):
        ...
        for batch_idx, sample in enumerate(train_loader):
            ...
            # Method 3
            img = sample[0].to(device)
            targets = [torch.autograd.Variable(ann.to(device), volatile=True) for ann in sample[1]]
